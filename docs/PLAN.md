...
**Clôture (2026-09-24, mise à jour après NewsEye).** Dix items : huit faits et vérifiés par run (`VR-1` à `VR-6`, `VR-8`, `VR-9`), deux mesurés et laissés à décision (`VR-7`, `VR-10`). Sur la presse en production (`hans`, rapport `H17`) : recadrage par ligne + `vision(attachment_scope="page")` 21,20 → 18,28 %, au niveau du script. Reste une
