...
| `VR-12` | **Le début (ou la fin) d'une correction vient de la ligne voisine, le reste est juste** — trois cas vus à l'œil (`hans`, H19) que ni la marge (la ligne reste plus proche d'elle-même), ni l'absorption (pas plus longue), ni `VR-11` ne voient : « TIR. » de la ligne suivante en tête (OCR17+, Corneille), « pouvoir » de la ligne du dessus (NewsEye, recadrage), une demi-ligne calquée sur la voisine livrée `corrected` (NewsEye, composite). Les comptes « 0 mal rattachée » de `VR-1`/`VR-7` sont ceux du proxy (ligne entière) | mesure puis garde | important | `core/guards.py` | `VR-11` | **à mesurer avant d'écrire** : si les k premiers (derniers) mots de la correction sont ceux d'une voisine et pas ceux de la source → refus ou renvoi en revue. À chiffrer d'abord : combien de corrections justes commencent légitimement comme la voisine (répliques « TIR. » / « ERA. », listes, titres) ; deux des trois cas sont déjà sortis en `review_required` par la règle des noms propres |
